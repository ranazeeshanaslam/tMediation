 ...
 * @version 1.0
 */
public class TerminusMediator32{
    ...
    static Hashtable elementHash;
    	...
    	*/
        ...
        PropertyConfigurator.configure(path + "conf/log_tmediator32.properties");
        Logger logger = Logger.getLogger("SwitchMediator");
        ...
        DBConnector dbConnector;
          ...
          try {
              conf = new MediatorConf(path +"conf/conf_tmediator32.properties");
          } catch (Exception ex1) {
            try {
         ...
         parms.setLogFilePath(path+"logs/");
          ...
          if (seprator == 2) {
			...
			try{
				...
				TerminusMediator32 ism = new TerminusMediator32();
				
				CollectSystemStatistics css = new CollectSystemStatistics(conn, logger);
				css.run();
				long Records = 0;
				if (Util.validateSystem(conn, logger)){
					Records = ism.mediateCDRFiles(conf, dbConnector, sep_string, logger, parms);
					//isRunning = false;
				}else{
          ...
          try {
             ...
             // stmt = conn.createStatement();
	          ...
	          for(int i=0; i<NoOfThreads; i++){
	        	TerminusCDRMediator32 mediator = new TerminusCDRMediator32();
	                        	  ...
	                        	  if (sdrfile.getFN_FILEID()> 0) {
	                        	    	...
	                        	    	boolean isAllocated=false;
			              	            ...
			              	            for(int i=0; i<NoOfThreads && !isAllocated ; i++){
			              	            	TerminusCDRMediator32 mediator = (TerminusCDRMediator32)ThreadArray.get(i);
			              	            	if (!mediator.isAlive()){
			              	        			mediator = new TerminusCDRMediator32(i+1, Filename, sdrfile, isSecondary,
