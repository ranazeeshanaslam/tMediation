 ...
 * @version 1.0
 */
public class QubeTalkCDRMediator {
...
boolean isRunning = false;
    ...
    public static void main(String argv[]) throws IOException {
    	...
    	try{
          ...
          PropertyConfigurator.configure(argv[0] + "conf/log_qubetoyotaCapitals.properties");
          ...
          try {
              conf = new MediatorConf(argv[0] +"conf/conf_qubetoyotaCapital.properties");
          } catch (Exception ex1) {
         ...
         parms.setErrSQLFilePath(argv[0]+"alarms/");
          ...
          ServerName = conf.getPropertyValue(conf.SERVER_NAME);
			...
			try{
				...
				QubeTalkCDRMediator mediator = new QubeTalkCDRMediator();
				
				CollectSystemStatistics css = new CollectSystemStatistics(conn, logger);
				css.run();
				long Records = 0;
				if (Util.validateSystem(conn, logger)){
					Records = mediator.mediateQubeTalkCDRFiles(conf, dbConnector, logger, parms);
				}else{
      ...
      public long mediateQubeTalkCDRFiles(MediatorConf conf, DBConnector dbConnector, Logger logger, MediatorParameters parms) {
		                                        	    ...
		                                        	    if (duplicate){
		                                        	    	...
		                                        	    	ICPNode inode = Util.identifyICPNode(inboundTrunk, "", "", Destination, Destination, true, ne, NodeIdentificationHash, NodeHash); 
			                                  	    		...
			                                  	    		if(CallerId.length()==4)
			                                  	    			CallerId="037962"+CallerId;
			                                  	    		else if(CallerId.length()>4 && CallerId.startsWith("60")){
