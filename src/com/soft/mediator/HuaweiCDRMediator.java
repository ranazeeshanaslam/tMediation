 ...
 * @version 1.0
 */
public class HuaweiCDRMediator {
...
boolean isRunning = false;
    ...
    public static void main(String argv[]) throws IOException {
         ...
         parms.setErrSQLFilePath(argv[0]+"alarms/");
          ...
          ServerName = conf.getPropertyValue(conf.SERVER_NAME);
			...
			try{
				...
				HuaweiCDRMediator mediator = new HuaweiCDRMediator();
				...
				//if (Util.validateSystem(conn, logger)){
					Records = mediator.mediateHuaweiCDRFiles(conf, dbConnector, logger, parms);
				//}else{
      ...
      public long mediateHuaweiCDRFiles(MediatorConf conf, DBConnector dbConnector, Logger logger, MediatorParameters parms) {
		                            	  ...
		                            	  while(true)
		                                   { //#1
		                                      ...
		                                      String HW_CALLEDGW="";
		                                       ...
		                                       HW_CSN=hex2dec(extractField(4,0,buffer));
		                                       HW_BILLTYPE=hex2dec(extractField(1,8,buffer));
		                                       String datestr=extractField(7,10,buffer);


			                           			//System.out.println("Start Time="+test);
	
			                           			String sdateTime = hex2dec(reverseBytes(datestr.substring(0, 4), 2)) + "-";
			                           			...
			                           			sdateTime += hex2dec(datestr.substring(12, 14));
			                           			String stime=sdateTime;
			                           			HW_STARTTIME="to_date('"+sdateTime +"','YYYY-MM-DD HH24:MI:SS')";
			                           				                           			
			                           			String Durstr=extractField(4,98,buffer);
			                        			Durstr=hex2dec(Durstr);
			                           			Durstr=String.valueOf(parseDuration(Durstr));
			                           			HW_CALLDURATION = Long.parseLong(Durstr);
			                           			//System.out.println("\n\n\nDuration (Long): " + HW_CALLDURATION + "\n\n");
			                           			
			                           			HW_CALLINGNUMBER=extractField(14,17,buffer);
			                           			HW_CALLINGNUMBER=discardStringAtRight(HW_CALLINGNUMBER.substring(4, HW_CALLINGNUMBER.length()), "F");
			                           			HW_CALLEDNUMBER=extractField(14,31,buffer);
			                        			HW_CALLEDNUMBER=discardStringAtRight(HW_CALLEDNUMBER.substring(4, HW_CALLEDNUMBER.length()), "F");
			                                    ...
			                                    String tgStr=extractField(2,94,buffer);
			                                    HW_TRUNKGROUPIN=Integer.parseInt(hex2dec(tgStr));
			                                    String tgStrout=extractField(2,96,buffer);
			                                    HW_TRUNKGROUPOUT=Integer.parseInt(hex2dec(tgStrout));
			                                    String tCode=extractField(1,108,buffer);
                                        	    ...
                                        	    if (duplicate){
                                        	    	...
                                        	    	sql = " insert into SDR_TBLHUAWEICDRS ( HW_CSN, HW_NETTYPE, HW_BILLTYPE, HW_STARTTIME, HW_ENDTIME, HW_CALLDURATION, HW_CALLINGNUMBER, HW_CALLEDNUMBER, HW_TRUNKGROUPIN, HW_TRUNKGROUPOUT, HW_CALLTYPE, HW_TERMINCODE,ne_elementid, HW_CALLINGK, HW_CALLINGW, HW_CALLEDGK, HW_CALLEDGW, FN_FILEID) "+
                                        	    			...
                                        	    			"  "+ network_element +",'','','','', "+ sdrfile.getFN_FILEID()+ ")" ;
				                                    logger.debug(sql);
		//                            	    		int isExecuted = 0;
		                                	    	logger.debug(sql);
		                                	    	int isExecuted = 0;
...
private String formatDate(String someDate){  //Pass the date in the format like 17.12.2008-11:02:44
  ...
  public static int parseDuration(String duration) {
	  ...
	  //System.out.println("\n\n\nDuration (int): " + (Integer.parseInt(duration) / 1000000) + "\n\n");
