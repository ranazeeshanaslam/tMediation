 ...
 * @version 1.0
 */
public class StandardInobelGWCDRMediator {
...
boolean isRunning = false;
    ...
    public StandardInobelGWCDRMediator() {
    }
    ...
    public static void main(String argv[]) throws IOException {
    	...
    	try{
          ...
          PropertyConfigurator.configure(argv[0] + "conf/log_standardInobelgw.properties");
          Logger logger = Logger.getLogger("InobelGWCDRMediator");

          MediatorConf conf;
          DBConnector dbConnector;
          
          try {
              conf = new MediatorConf(argv[0] +"conf/conf_standardInobelgw.properties");
          } catch (Exception ex1) {
         ...
         parms.setErrSQLFilePath(argv[0]+"alarms/");
          ...
          ServerName = conf.getPropertyValue(conf.SERVER_NAME);
			...
			try{
				...
				StandardInobelGWCDRMediator mediator = new StandardInobelGWCDRMediator();
				
				CollectSystemStatistics css = new CollectSystemStatistics(conn, logger);
				css.run();
				long Records = 0;
				if (Util.validateSystem(conn, logger)){
					Records = mediator.mediateInobelGWCDRFiles(conf, dbConnector, logger, parms);
				}else{
      ...
      public long mediateInobelGWCDRFiles(MediatorConf conf, DBConnector dbConnector, Logger logger, MediatorParameters parms) {
          ...
          try {
              ...
              if (!dir.isDirectory() || !destdir.isDirectory()) {
                  ...
                  for (int j = 0; j < FileNames.length; j++) {
                      ...
                      if(Filename.contains("digi") || Filename.contains("DIGI")){
                    	  network_element=48;////
                      }
                      else if(Filename.contains("tm") || Filename.contains("TM")){
                    	  network_element=47;////
                      }
                      else
                          continue;
...
--Dial,Zap/g1/0137741870|60|oTW,2009-11-03 17:45:41,2009-11-03 17:45:57,2009-11-03 17:46:46,65,49,ANSWERED,DOCUMENTATION,0351635318
		                                     ...
		                                     */
		                                       ...
		                                       String UserName = "";      	//2
		                                       String Password= "";   		//3
		                                       String AccessNumber = "";     	//4
		                                       String CallingNumber = "";    	//5
		                                       String CalledNumber = "";	//6
		                                       String ActualDuration = "";	//7
		                                       String Duration = "";     	//8
		                                       String IngressTrunk = "";	//9
		                                       String EgressTrunk = "";  	//10
		                                       erroroccured =false;
		                                       if (linecount == 0 && ignoreFirstLine && newLine.length() > 0){
		                                          ...
		                                          while (i < lineLength) {
		                                           ...
		                                           try{
		                                                  switch (wordscount) {
			                                                  ...
			                                                  case 2: UserName =  value.trim();  break;                     //2
			                                                  case 3: Password =  value.trim();  break;                     //3
			                                                  case 4: AccessNumber =  value.trim();  break;      		//4
			                                                  case 5: CallingNumber = value.trim(); break;                  //5
			                                                  case 6: CalledNumber =  value.trim();  break; 	      	//6
			                                                  case 7: ActualDuration =  value.trim();  break; 	      	//7
			                                                  case 8: Duration =  value.trim();  break;                     //8
			                                                  case 9: IngressTrunk =	value.trim(); break; 	 	//9
			                                                  case 10: EgressTrunk =  value.trim();  break;   		//10
		                                              ...
		                                              i++;
		                                           	...
		                                           	if (CalledNumber != "" && CalledNumber.length() != 0) {
                                                                        DuplicateSDR duplicatesdr = new DuplicateSDR(Calldate+UserName+Password+AccessNumber+CallingNumber+CalledNumber, Calldate, network_element, sdrfile.getFN_FILEID());
		                                        	    ...
		                                        	    if (duplicate){
		                                        	    	...
		                                        	    	DupCDRs++;
		                                        	    	DupCDRsInFile++;
		                                        	    	logger.debug(" Duplicate CDRs UniqueID:"+Calldate+UserName);
		                                        	    }else{
		                                        	    	...
		                                        	    	if (ActualDuration.length()==0) ActualDuration="0";
			            						...
			            						try{
			            							CalledNumber = CalledNumber.substring(CalledNumber.indexOf("/", 4)+1, CalledNumber.indexOf("|", 0));
			                                        	    	...
			                                        	    	ICPNode inode = Util.identifyICPNode(IngressTrunk, "", "", CalledNumber, CalledNumber, true, ne, NodeIdentificationHash, NodeHash);
...
//                                                                      logger.debug(sql);
			                            			    	...
			                            			    	" values ( '"+UserName+"',  "+CalldateTime+", "+Duration+", " +
			                            	    	  		" '"+CallingNumber+"', '"+CallingNumber+"', '"+CalledNumber+"', '"+TCalledNumber+"'," +
			                            	    	  		" '"+AccessNumber+"', '', '"+IngressTrunk+"', '"+EgressTrunk+"'," +
			                            	    	  		" '',  '', '', '', '"+orgCalledNumber+"'," +
			                            	    	  		" '', '', '', '', " +
			                            	    	  		"  "+network_element+", "+sdrfile.getFN_FILEID()+", 0,"+AS_CHARGE+","+iNodeID+","+eNodeID+","+ActualDuration+")" ;
			                            		    	logger.debug(sql);
//	                                        	    
		                                        	    	int isExecuted = 0;
				                                            try {
