 ...
 * @version 1.0
 */
public class NextoneCDRMediator implements Mediator {
         ...
         parms.setLogFilePath(path+"logs/");
          ...
          if (seprator == 2) {
			...
			try{
				...
				NextoneCDRMediator ism = new NextoneCDRMediator();
				
				CollectSystemStatistics css = new CollectSystemStatistics(conn, logger);
				css.run();
				long Records = 0;
				if (Util.validateSystem(conn, logger)){
					Records = ism.mediateNSSWCDRFiles(conf, dbConnector, sep_string, logger, parms);
					isRunning = false;
				}else{
          ...
          try {
             ...
             // stmt = conn.createStatement();
	          ...
	          for(int i=0; i<NoOfThreads; i++){
          		NextoneMediator mediator = new NextoneMediator();
	                        	  ...
	                        	  if (sdrfile.getFN_FILEID()> 0) {
	                        	    	...
	                        	    	boolean isAllocated=false;
			              	            ...
			              	            for(int i=0; i<NoOfThreads && !isAllocated ; i++){
			              	            	NextoneMediator mediator = (NextoneMediator)ThreadArray.get(i);
			              	            	if (!mediator.isAlive()){
			              	        			mediator = new NextoneMediator(i+1, Filename, sdrfile, isSecondary,
