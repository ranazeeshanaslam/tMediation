...
public class TelesC5Mediator_new_badar {

    boolean isRunning = false;

    //handling call forwarding, and other services.......
    ...
    public TelesC5Mediator_new_badar() {
    }
    ...
    public static void main(String argv[]) throws IOException {
		  ...
		  try{
			...
			TelesC5Mediator_new_badar mediator = new TelesC5Mediator_new_badar();
      ...
      public long mediatetelesC5CDRFiles(MediatorConf conf, DBConnector dbConnector, Logger logger, MediatorParameters parms) {
          ...
          try {
              ...
              logger.info("GMT_HOURS ="+gmtHours);
		                                       ...
		                                       if (linecount == 0 && ignoreFirstLine && newLine.length() > 0){
		                                          ...
		                                          while (i < lineLength) {
		                                            ...
		                                            if(CallProgressState == null){
		                                            	CallProgressState = "";
		                                            	calltypeID=1;
		                                            }else if(CallProgressState.length()>0){
		                                            	if(CallProgressState.contains("CFU")){
		                                            		calltypeID=3;
		                                            	}else if(CallProgressState.contains("RDIR")){
		                                            		calltypeID=3;
		                                            	}
		                                            }
		                                            logger.info("calltypeID:"+calltypeID);
                                                    ...
                                                    int fromNolength=CallingNumber.length();
                                                    logger.info("OrignalFromNumber:"+OrignalFromNumber);
                                                    String inDomain="";
                                                    String outDomain="";
                                                    int tmp3=OrignalFromNumber.indexOf("@");
                                                    if(tmp3>0){
                                                    	inDomain=OrignalFromNumber.substring(tmp3+1,OrignalFromNumber.length() );
                                                    }
                                                    logger.info("inDomain:"+inDomain);
                                                    int tmp4=OrignalDialerNumber.indexOf("@");
                                                    if(tmp4>0){
                                                    	outDomain=OrignalDialerNumber.substring(tmp4+1,OrignalDialerNumber.length() );
                                                    }	
                                                    logger.info("outDomain:"+outDomain);
                                                    ...
                                                    if(tmp > 0 && tmp2 > 0 && tmp > tmp2){
                                                    	extNumber=OrignalDialerNumber.substring(tmp2+1, tmp);
	                                                    if(extNumber.startsWith("+"))
	                                                    	extNumber=extNumber.substring(1);
                                                    } //if(tmp > 0 && tmp2 > 0 && tmp > tmp2)
                                                    logger.info("extNumber:"+extNumber);
                                                    String terID="";
                                                    tmp=TerminatorId.indexOf("@");
                                                    tmp2=TerminatorId.indexOf(":");
                                                    if(tmp > 0 && tmp2 > 0 && tmp > tmp2){
                                                    	terID=TerminatorId.substring(tmp2+1, tmp);
	                                                    
                                                    } //if(tmp > 0 && tmp2 > 0 && tmp > tmp2)
                                                    logger.info("terID:"+terID);
                                                    logger.info("TerminatorNumber:"+TerminatorNumber);
                                                    
                                                    if(TerminatorNumber != null && TerminatorNumber.length() > 0){
                                                    	if(calltypeID==2 || calltypeID==3 || TerminatorNumber.contains(";")){
                                                    		 String CalledArr[] = TerminatorNumber.split(";");
                                                    		CalledNumber=CalledArr[0];
                                                    		if(calltypeID==2 || calltypeID==3){
	                                                    		if(CalledArr.length>2)
	                                                    			ForwordedNumber=CalledArr[2];
	                                                    		else if(CalledArr.length>1)
	                                                    			ForwordedNumber=CalledArr[1];
                                                    		}
                                                    	}
                                                    	else                                                    	
                                                    		CalledNumber=TerminatorNumber;
                                                    	logger.info("CalledNumber 1:"+CalledNumber);
                                                    	logger.info("ForwordedNumber 1:"+ForwordedNumber);
                                                    	
                                                    	if(CalledNumber.startsWith("+191852") || CalledNumber.startsWith("191852")|| CalledNumber.startsWith("191952") || CalledNumber.startsWith("+191952")){
    	                                                //if(CalledNumber.startsWith("+191852") || CalledNumber.startsWith("191852")){
	                                                    	if(CalledNumber.startsWith("+"))
	                                                    		CalledNumber=CalledNumber.substring(7);
	                                                    	else
	                                                            CalledNumber=CalledNumber.substring(6);
	                                                    }
                                                    	logger.info("CalledNumber 2:"+CalledNumber);
	                                                    if(ForwordedNumber.length()>0){
	                                                    	if(ForwordedNumber.startsWith("+191852") || ForwordedNumber.startsWith("191852")|| ForwordedNumber.startsWith("191952") || ForwordedNumber.startsWith("+191952")){
	    	                                                //if(ForwordedNumber.startsWith("+191852") || ForwordedNumber.startsWith("191852")){
		                                                    	if(ForwordedNumber.startsWith("+"))
		                                                    		ForwordedNumber=ForwordedNumber.substring(7);
		                                                    	else
		                                                    		ForwordedNumber=ForwordedNumber.substring(6);
		                                                    }
		                                                    else if(ForwordedNumber.startsWith("+"))
		                                                    	ForwordedNumber=ForwordedNumber.substring(1);
	                                                    }
	                                                    logger.info("ForwordedNumber 2:"+ForwordedNumber);
                                                        ...
                                                        if(tmp > 0 && tmp2 > 0 && tmp > tmp2){
    	                                                    ...
    	                                                    if(CalledNumber.startsWith("0092"))
    	                                                    	CalledNumber=CalledNumber.substring(2);
    	                                                    if(fromNolength<=6 && CalledNumber.length()>6 ){
    	                                                    	CalledNumber=CalledNumber.substring(1);
    	                                                    	if(CalledNumber.startsWith("0")){
    	                                                        	CalledNumber=CalledNumber.substring(1);
    	                                                			CalledNumber="92"+CalledNumber;
    	                                                        }
                                                        ...
                                                        logger.info("CalledNumber 3:"+CalledNumber);
                                                    }
                                                    
                                                    
                                                    if(extNumber.length()<=6 && fromNolength<=6){
                                                    	CalledNumber=extNumber;
                                                    }
                                                    if(terID.length()<=6 && CalledNumber.length()>6 ){
                                                		if(CalledNumber.endsWith(terID)){
                                                			CalledNumber=CalledNumber.substring(0,(CalledNumber.length()- terID.length()));
                                                		}
                                                		logger.info("CalledNumber 4:"+CalledNumber);
//                                                    		if(tempUserName.endsWith(terID)){
//                                                    			tempUserName=tempUserName.substring(0,(tempUserName.length()- terID.length()));
//                                                    		}
                                                    }
                                                    
                                                    if(CalledNumber.startsWith("+"))
                                                        CalledNumber=CalledNumber.substring(1);
                                                    logger.info("CalledNumber 5:"+CalledNumber);
                                                    ////////////////////////////////////////////////
                                                    if(OrignatorId.contains("lbo")){
                                                    	
                                                    	tmp=OrignatorId.indexOf("@");
                                                        tmp2=OrignatorId.indexOf(":");
                                                        if(tmp > 0 && tmp2 > 0 && tmp > tmp2){
    	                                                    orignatorNumber=OrignatorId.substring(tmp2+1, tmp);
    	                                                    if(orignatorNumber.startsWith("+")){
    	                                                    	orignatorNumber=orignatorNumber.substring(1);
    	                                                    }
    	                                                    logger.info("orignatorNumber1:"+orignatorNumber);
    	                                                    logger.info("CallingNumber with ext:"+CallingNumber);
    	                                                    int orlength=orignatorNumber.length()-3;
    	                                                    orignatorNumber=orignatorNumber.substring(0,orlength);
    	                                                    logger.info("orignatorNumber2:"+orignatorNumber+" length:"+orlength);
    	                                                    orlength=orignatorNumber.length();
    	                                                    if(CallingNumber.length()>orlength){
    	                                                    	CallingNumber=CallingNumber.substring(0,orlength-1);
    	                                                    	logger.info("CallingNumber without ext:"+CallingNumber);
    	                                                    }
                                                        }
                                                        
                                                    }//if(OrignatorId.contains("lbo"))
                                                    
                                                    logger.info("CalledNumber3:"+CalledNumber);
                                                    
                                                    int toNolength=CalledNumber.length();
                                                    logger.info("fromNolength:"+fromNolength);
                                                    logger.info("toNolength:"+toNolength);
                                                    if(fromNolength<=6 && toNolength<=6 && calltypeID!=3 ){
                                                    	
                                                    	calltypeID=4;
                                                    	
                                                    	
                                                    }else if(fromNolength<=6 && toNolength>6  ){
                                                    	calltypeID=5;
                                                    	//CalledNumber=CalledNumber.substring(1);
                                                    	//logger.info("CalledNumberin:"+CalledNumber);
                                                    }
                                                    if(CalledNumber.startsWith("0092")){
                                                    	CalledNumber.substring(2);
                                                    }
                                                     
                                                    logger.info("CalledNumber:"+CalledNumber);
                                                    logger.info("calltypeID:"+calltypeID);
...
//                                                            	UserName=tempUserName;
                                                            ...
                                                            sql+=   "(tc_ischargeable,tc_versionid,tc_logtype,tc_sequencenumber," +
                                                                ...
                                                                "TC_EVENTDATE,  NE_ELEMENTID, FN_FILEID,TC_SSWCALLTYPEID,TC_FORWORDNUMBER) values ( "+Ischargeable+",  '"+VersionId+"', '"+Logtype+"', '"+SequenceNumber+"', " +
                                                                ...
                                                                " '"+CallingNumber+"', '"+CalledNumber+"',"+donedate+","+ network_element+","+sdrfile.getFN_FILEID()+","+calltypeID+",'"+ForwordedNumber+"' )" ;
	                                                        logger.debug(sql);
		                                        	    	int isExecuted = 0;
				                                            try {
		                              ...
		                              if (sdrfile!= null && sdrfile.getFN_FILEID()> 0) {
		                            	  isSuccess = sdrfile.updateSDRFile(conn, logger, sdrfile, CDRinFileCount, CDRinFileInserted, DupCDRsInFile,billableCDRs);
		                              }else{
		                            	  logger.info("sdr file is null");
		                              }
