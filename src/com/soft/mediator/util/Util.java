...
public class Util {
    ...
    public static ApplicationServer getApplicationServer(Connection conn, String Name, String IP, Logger log)throws SQLException, NullPointerException {
	  ...
	  public static AppProcHistory getNewServerProcess_Old(Connection conn, String ServerName, String ServerIP, Logger log) throws SQLException {
	  ...
	  public static AppProcHistory getNewServerProcess(Connection conn, String ServerName, String ServerIP, Logger log) throws SQLException {
		    ...
		    try {
		        ...
		        sql = "INSERT INTO SC_TBLAPPPROCHISTORY (APH_PROCNAME, AS_SERVERID, APH_PROCDATE, "
		        	+ "APH_ISSUCCESS, APH_PROCESSEDRECORDS, APH_ERRORRECORDS, APH_TIMECONSUMEDMS) "
		            + "VALUES (?, ?, SYSDATE, ?, ?, ?, ?)";
			        
			        
		        try (PreparedStatement insertStmt = conn.prepareStatement(sql, new String[] { "APH_PROCID" })) {
		            insertStmt.setString(1, process.getProcessName());
		            insertStmt.setLong(2, process.getServerID());
		            insertStmt.setInt(3, process.getisSuccess());
		            insertStmt.setLong(4, process.getProcessedRecords());
		            insertStmt.setLong(5, process.getErrorRecords());
		            insertStmt.setLong(6, process.getTimeConsumed());
		            insertStmt.executeUpdate();

		            // Retrieve the generated auto-incremented ID
		            try (ResultSet generatedKeys = insertStmt.getGeneratedKeys()) {
		                if (generatedKeys.next()) {
		                    process.setProcessID(generatedKeys.getLong(1));
		                }
		            }
		        }
	  ...
	  public static ArrayList<BNumberRule> getBNumberRules(Connection conn, Logger logger)throws SQLException, 
		 NullPointerException {
			
			ArrayList<BNumberRule> taskList = new ArrayList<BNumberRule>();
	  ...
	  /*
	    public BNumberRuleForC5(int id, String srcstring,String stringPos,int stringLength,String lengthOpt, int lengthtoconsider,
	    		String replaceWith, int stopProcessing, int ForANum, int ForBNum, 
			    			int rulePriority,int isDisabled,long SysUserID,String SysUserIP,String InsertDate){
	    
	   */

	  public static ArrayList getBNumberRulesForC5(Connection conn, Logger logger)throws SQLException, 
		 NullPointerException {
			
			ArrayList taskList = new ArrayList();
			String sql =" SELECT BNR_BNRULEID,BNR_SrcString, BNR_StringPos, BNR_StringLength, BNR_LengthOpt, BNR_LENGTHTOCONSIDER, " +
						" BNR_DOREPLACE, BNR_ReplaceWith, BNR_StopProcessing, BNR_FORANUM, BNR_FORBNUM, BNR_RULEPRIORITY,BNR_ISDISABLED," +
						" SU_SysUserID, SU_SysUserIP, SU_InsertDate "+
						" from TMR_TBLBNUMBERRULESFORC5 order by BNR_RULEPRIORITY, length(BNR_SrcString) desc ";
			logger.debug(sql);
			PreparedStatement pstmt = null;
			ResultSet rs = null;
			try {
				...
				if (start) {
					while (start) {
						BNumberRuleForC5 bnr = new BNumberRuleForC5(rs.getInt("BNR_BNRULEID"),rs.getString("BNR_SrcString"),rs.getString("BNR_StringPos"),
											rs.getInt("BNR_StringLength"), rs.getString("BNR_LengthOpt"),rs.getInt("BNR_LENGTHTOCONSIDER"),
											rs.getInt("BNR_DOREPLACE"), rs.getString("BNR_ReplaceWith"),rs.getInt("BNR_StopProcessing"),
											rs.getInt("BNR_FORANUM"), rs.getInt("BNR_FORBNUM"),
											rs.getInt("BNR_RULEPRIORITY"),rs.getInt("BNR_ISDISABLED"),
											rs.getLong("SU_SysUserID"), rs.getString("SU_SysUserIP"), rs.getString("SU_InsertDate"));
						taskList.add(bnr);
						start = rs.next();
					}
	  ...
	  public static Hashtable<String, ICPNodeIdentification> getICPNodeIdentifications(Connection conn, Logger log) {
			
			Hashtable<String, ICPNodeIdentification> nodeList = new Hashtable<String, ICPNodeIdentification>();
		    String sql = "";
	  ...
	  public static Hashtable<Integer, ICPNode> getICPNodes(Connection conn, Logger log) {
			
			Hashtable<Integer, ICPNode> nodeList = new Hashtable<Integer, ICPNode>();
			String sql = "";
	  ...
	  public static ICPNode identifyICPNode(String TrunkValue, String Techprefix, String CallRegValue, String CalledNumber, String RxCalledNumber, boolean ingress, NetworkElement el, Hashtable<String, ICPNodeIdentification> identhash, Hashtable Nodeshash ) throws Exception {
	  
		 return identifyICPNode(TrunkValue,  Techprefix,  CallRegValue,  CalledNumber,  RxCalledNumber, ingress,  el,  identhash, Nodeshash,"" ); 
	  }
	  
	  public static ICPNode identifyICPNode(String TrunkValue, String Techprefix, String CallRegValue, String CalledNumber, String RxCalledNumber, boolean ingress, NetworkElement el, Hashtable<String, ICPNodeIdentification> identhash, Hashtable Nodeshash, String Operator ) throws Exception {
		    ...
		    if (ingress && TrunkValue.length()==0 && CalledNumber.length()==0 )
		    	...
		    	if(Operator.equals("DialogicSBC")){
		    		
		    		if(TrunkValue!=null && TrunkValue.length()>0) {		    			
		    			if (ingress){
		    				DebugMsg +="\n Getting Ingress Node Information by TrunkValue from hash";
		    				nodeident = identhash.get(TrunkValue.trim().toUpperCase());
		    			}else if(!ingress){
		    				DebugMsg +="\n Getting Egress Node Information by TrunkValue from hash";
		    				nodeident = identhash.get(TrunkValue.toUpperCase());
		    			}
		    		}   
			    
		    	} else if(Operator.equals("Multinet")){
			    	...
			    	if (ingress && CalledNumber.length()>0 && Techprefix.length() == 0){
			    		...
			    		nodeident = identifyICPNodeByTechPrefix(identhash, CalledNumber); 
			    	}else if (!ingress && CalledNumber.length()>0 && Techprefix.length() > 0 && Operator.equalsIgnoreCase("Alsard") ){
			    		DebugMsg +="\n Getting Egress Node Information by Technical Prefix ";
			    		nodeident = identifyICPNodeByTechPrefix(identhash, CalledNumber); 
			    	}
			    	if (nodeident.getIdentificationID() > 0){
			    		StripPrefix = true;
			    		DebugMsg +="\n Technical Prefix Found : "+nodeident.getIdentificationvalue();
			    		DebugMsg +="\n StripPrefix : "+StripPrefix;
			    	}else{ //#2
			    		DebugMsg +="\n Technical Prefix Not Found ";
			    		if (ingress){ // for ingress partner node
			    			DebugMsg +="\n Getting Ingress Node Information ";
			    			//System.out.println(" Getting Ingress Node Information ");
			    				if (el.getVendorName().equalsIgnoreCase("Huawei") && TrunkValue.length()>0){
			    				...
			    				//	nodeident = (ICPNodeIdentification)identhash.get(Techprefix.toUpperCase());
			    					...
			    					if ((nodeident == null || nodeident.getIdentificationID()==0) && Operator.equalsIgnoreCase("Alsard") && CallRegValue.length()>0){
			    						nodeident = (ICPNodeIdentification)identhash.get(CallRegValue.toUpperCase());
			    					}
			    			...
			    			DebugMsg +=" \nGetting Egress Node Information ";
			    			if (el.getVendorName().equalsIgnoreCase("Huawei")  && TrunkValue.length()>0){
		    					nodeident = (ICPNodeIdentification)identhash.get(TrunkValue.toUpperCase());
		    				...
		    				//	nodeident = (ICPNodeIdentification)identhash.get(TrunkValue.toUpperCase());
		    					...
		    					if ((nodeident == null || nodeident.getIdentificationID()==0) && TrunkValue.length()>0){
		    						DebugMsg +=" \nTrunkValue : "+TrunkValue.toUpperCase();
		    						nodeident = (ICPNodeIdentification)identhash.get(TrunkValue.toUpperCase());
		    					}
...
//		    	Util.writeDebugLog("./logs/p-log.log", "TrunkValue :"+TrunkValue);
//		    	Util.writeDebugLog("./logs/p-log.log", "Node ID :"+nodeident.getNodeID());
     		   
		    	if (nodeident != null && nodeident.getNodeID() > 0){
		    		
		    		node = (ICPNode)Nodeshash.get(nodeident.getNodeID());
		    		
//		    		Util.writeDebugLog("./logs/p-log.log", "Node Idnet Found NODE ID:"+node.getNodeID());
	  ...
	  public static BNumberRuleResult applyBNumberRules(String CalledNumber, ArrayList list, ICPNode node, boolean isC5, boolean isIngress) throws Exception {
		   ...
		   DebugMsg = "CalledNumber="+CalledNumber;
		   DebugMsg = "list.size()="+list.size();
	       ...
	       if (list != null && list.size()>0 && CalledNumber.length() > 0){
	 	      	int LastPriority = 0;
	 		   	for (int i=0; i<list.size() && !stopProcessing; i++){
	 		   	    ...
	 		   	    //System.out.println("node.getPartnerID()="+node.getPartnerID());
	 		        //System.out.println("rule.getPartnerID()="+rule.getPartnerID());
	 		   		...
	 		   		//System.out.println("ApplyRule="+ApplyRule);
	 		   		if (rule != null && ApplyRule && rule.getRulePriority()>= LastPriority ){
	 		   	   	DebugMsg = "rule.getSrcString()="+rule.getSrcString();
	 		   	   	//System.out.println("rule.getSrcString()="+rule.getSrcString());
	 		   	   	DebugMsg = "rule.getStringPos()="+rule.getStringPos();
	 		   	   	//System.out.println("rule.getStringPos()="+rule.getStringPos());
	 		   	   	//System.out.println("rule.getReplaceWith()="+rule.getReplaceWith());
	 		   			...
	 		   			}else if ((rule.getSrcString().length()> 0 || rule.getReplaceWith().length()>0 ) && rule.getStringPos().equalsIgnoreCase("Postfix")){
	       ...
	       result.setLogMessage(DebugMsg);
	 	   return result;
	   }
	  
	  public static BNumberRuleResult applyBNumberRulesForC5(String NumberToCheck, ArrayList list, boolean isBNumber) throws Exception {
		   ...
		   DebugMsg += "\nRule List.size()="+list.size();
		   //System.out.println("Rule List.size()="+list.size());
		   
	       if (list != null && list.size()>0 && NumberToCheck.length() > 0){
	 	      	int LastPriority = 0;
	 		   	for (int i=0; i<list.size() && !stopProcessing; i++){
	 		   		BNumberRuleForC5 rule = (BNumberRuleForC5)list.get(i);
	 		   		boolean ApplyRule = true;
		 		   	if ( !isBNumber && rule.getForANum() == 0 ){
						ApplyRule = false;
	 		   			DebugMsg += "\nThis Rule is for A-Number";
					}else if (isBNumber && rule.getForBNum() == 0){
						ApplyRule = false;
	 		   			DebugMsg += "\nThis Rule is for B-Number";
					}else if (rule.getStringLength() > 0 && rule.getLengthOpt().equalsIgnoreCase("=") && rule.getStringLength() != Number.length()){
						ApplyRule = false;
	 		   			DebugMsg += "\nNumber Length mis-match: Rule :"+rule.getLengthOpt()+" "+rule.getStringLength()+" Actual Length :"+Number.length();
					}else if (rule.getStringLength() > 0 && rule.getLengthOpt().equalsIgnoreCase("<>") && rule.getStringLength() == Number.length()){
						ApplyRule = false;
	 		   			DebugMsg += "\nNumber Length mis-match: Rule :"+rule.getLengthOpt()+" "+rule.getStringLength()+" Actual Length :"+Number.length();
					}else if (rule.getStringLength() > 0 && rule.getLengthOpt().equalsIgnoreCase("<=") &&  Number.length() > rule.getStringLength() ){
	 		   		...
	 		   		//System.out.println("ApplyRule="+ApplyRule);
	 		   		if (rule != null && ApplyRule && rule.getRulePriority()>= LastPriority ){
	 		   	   	DebugMsg += "\nrule.getSrcString()="+rule.getSrcString();
	 		   	   	//System.out.println("rule.getSrcString()="+rule.getSrcString());
	 		   	   	DebugMsg += "\nrule.getStringPos()="+rule.getStringPos();
	 		   	   	//System.out.println("rule.getStringPos()="+rule.getStringPos());
	 		   	   	DebugMsg += "\nrule.getLengthToConsider()="+rule.getLengthToConsider();
	 		   	   	//System.out.println("rule.getLengthToConsider()="+rule.getLengthToConsider());
 		   	   	   	DebugMsg += "\nrule.getReplaceWith()="+rule.getReplaceWith();
 		   	   	   	//System.out.println("rule.getReplaceWith()="+rule.getReplaceWith());
 		   	   	   	
	 		   			LastPriority = rule.getRulePriority();
	 		   			if ( (rule.getSrcString().length()> 0 || rule.getReplaceWith().length()>0 ) && rule.getStringPos().equalsIgnoreCase("Equal")){
	 		   				if (Number.equalsIgnoreCase(rule.getSrcString())){
	 		   					if (rule.getLengthToConsider() > 0 && Number.length() > rule.getLengthToConsider())
	 		   						Number = Number.substring(0, rule.getLengthToConsider());
	 		   					if (rule.getdoReplace()==1)
	 		   						Number = rule.getReplaceWith();
	 		   				}
	 		   			}else if ((rule.getSrcString().length()> 0 || rule.getReplaceWith().length()>0 ) && rule.getStringPos().equalsIgnoreCase("Prefix")){
		 		   			if (Number.startsWith(rule.getSrcString())){
	 		   				...
	 		   				if (Number.endsWith(rule.getSrcString())){
	 		   					if (rule.getLengthToConsider() > 0 && Number.length() > rule.getLengthToConsider())
	 		   						Number = Number.substring(0, rule.getLengthToConsider());
	 		   					if (rule.getdoReplace()==1)
	 		   						Number = Number.substring(0, Number.length()-rule.getSrcString().length())+rule.getReplaceWith();
	 		   				}
	 		   			}else if ((rule.getSrcString().length()> 0 || rule.getReplaceWith().length()>0 ) && rule.getStringPos().equalsIgnoreCase("Contain")){
	 		   				if (Number.contains(rule.getSrcString())){
	 		   					if (rule.getLengthToConsider() > 0 && Number.length() > rule.getLengthToConsider())
	 		   						Number = Number.substring(0, rule.getLengthToConsider());
	 		   					if (rule.getdoReplace()==1)
	 		   						Number = Number.replaceAll(rule.getSrcString(), rule.getReplaceWith());
	 		   ...
	 		   DebugMsg +="\n Original Number ="+NumberToCheck+"   "+
			   			 " New Number ="+Number;
	 		   //System.out.print("BNumberRule Log:"+DebugMsg);	
	 		 // writeDebugLog("LoggingBNumber",DebugMsg);
	       }// end of else
	       BNumberRuleResult result = new  BNumberRuleResult();
	       result.setNumber(Number);
	       result.setStopProcessing(stopProcessing);
		...
		public static  Hashtable<Integer, NetworkElement> getNetworkElements(Connection conn, Logger log)throws SQLException, ClassNotFoundException, NullPointerException{

			  Hashtable<Integer, NetworkElement> hash = new Hashtable<Integer, NetworkElement>();
		      ...
		      log.debug("Network Element Hash Size :"+hash.size());
		      return hash;
		}
		...
		public static  ArrayList getTablesList(Connection conn, Logger log)throws SQLException, ClassNotFoundException, NullPointerException{

			  ArrayList  list=new ArrayList();
		      String sql = " SELECT RD_TableID, RD_TableName, RD_TruncateTable, RD_WhereClause, RD_TableKey, RD_isDisabled from  SC_TblRefreshData order by  RD_TableName ";
		      log.debug(sql);
		      PreparedStatement pstmt = null;
		      ResultSet rs = null;
		      try {
		          ...
		          if(start){
		              while(start){
		            	  RefreshData sa = new RefreshData(rs.getInt("RD_TableID"), rs.getString("RD_TableName"), rs.getInt("RD_TruncateTable"),
		            			  rs.getString("RD_WhereClause"), rs.getString("RD_TableKey"),rs.getInt("RD_isDisabled"));
		            	  list.add(sa);
		                  start = rs.next();
		              }
		          }
		      } catch (SQLException ex) {
		    	  log.error("SQL Exception in getTablesList :"+ex.getMessage()+"\n"+sql);
		      }catch (Exception ex) {
		          log.error(" Exception :"+ex.getMessage());
		      }finally{
		          try {
		...
		public static  SystemStatistics getSysStatistics(Connection conn, Logger log)throws Exception{
		  ...
		  public static int updateStats(Connection conn, SystemStatistics ss, Logger log) throws Exception {
			...
			public static  String encryptOneWay(String data) throws java.security.NoSuchAlgorithmException {
			...
			public static  String ConvertToHexString(byte[] bytes) {
			 ...
			 public static void adjustOpenCursors(Connection conn, int newLimit) {
		        Statement stmt = null;

		        try {
		            stmt = conn.createStatement();

		            // Construct the SQL command to set the OPEN_CURSORS parameter
		            String sql = "ALTER SYSTEM SET OPEN_CURSORS = " + newLimit + " SCOPE=BOTH";

		            // Execute the command
		            stmt.execute(sql);

		            System.out.println("Successfully set OPEN_CURSORS to " + newLimit);
		        } catch (SQLException e) {
		            e.printStackTrace();
		            System.err.println("Failed to set OPEN_CURSORS: " + e.getMessage());
		        } finally {
		            // Close resources
		            if (stmt != null) {
		                try {
