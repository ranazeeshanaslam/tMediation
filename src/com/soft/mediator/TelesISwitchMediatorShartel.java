 ...
 * @version 1.0
 */
public class TelesISwitchMediatorShartel {
    ...
    public TelesISwitchMediatorShartel() {
    ...
    public static void main(String argv[]) throws IOException {	
    	...
    	*/
        ...
        Logger logger = Logger.getLogger("TelesISwitchMediator");
         ...
         parms.setLogFilePath(path+"logs/");
          ...
          String sep_string = ",";
			...
			try{
				...
				TelesISwitchMediatorShartel ism = new TelesISwitchMediatorShartel();
				CollectSystemStatistics css = new CollectSystemStatistics(conn, logger);
				css.run();
				long Records = 0;
				//if (Util.validateSystem(conn, logger)){
					Records = ism.mediateTelesISwitchCDRsShartel(conf, dbConnector, sep_string, logger, parms);
				//}else{
				...
				if (conn == null)
					logger.error("Unable to connect to database :"+ex.getMessage());
				else
					logger.error("Exception in getting process detail :"+ex.getMessage());
			}
			finally{
				try{
					if(conn!=null){
      ...
      public long mediateTelesISwitchCDRsShartel(MediatorConf conf, DBConnector dbConnector,
          ...
          try {
              ...
              */
	          ...
	          for(int i=0; i<NoOfThreads; i++){
	        	  TelesiSwitchCDRMediatorShartel mediator = new TelesiSwitchCDRMediatorShartel();
            	  ...
            	  if (dir == null || destdir == null){
	                  ...
	                  for (int j = 0; j < FileNames.length; j++) {
	                      ...
	                      if (Filename.length() > 5 && Filename.endsWith(sourceFileExt)) {
	                          ...
	                          int instanceid=0;
	                          if(CDRFilename.contains("CE02"))
	                        	  instanceid=0;

	                          if(CDRFilename.contains("CE04"))
	                        	  instanceid=1;

	                          if(CDRFilename.contains("CE01"))
	                        	  instanceid=2;
	                          if(CDRFilename.contains("CE03"))
	                        	  instanceid=3;
	                      ...
	                      //    sdrfile = sdrfile.getSDRFile(conn, logger, 0, CDRFilename, ne.getElementID(), isSecondary);
	                          sdrfile = sdrfile.getSDRFile(conn, logger, 0, CDRFilename, ne.getElementID(), instanceid);
	                          if (sdrfile.getFN_FILEID() == 0){
	                        	  ...
	                        	  sdrfile = sdrfile.insertSDRFile(conn, logger, CDRFilename,  ne.getElementID(), instanceid, process.getProcessID());
	                        	  conn.commit();
	                          }
	                        	  ...
	                        	  if (sdrfile.getFN_FILEID()> 0) {
	                        	    	...
	                        	    	boolean isAllocated=false;
			              	            ...
			              	            for(int i=0; i<NoOfThreads && !isAllocated ; i++){
			              	            	TelesiSwitchCDRMediatorShartel mediator = (TelesiSwitchCDRMediatorShartel)ThreadArray.get(i);
			              	            	if (!mediator.isAlive()){
			              	        		//	mediator = new TelesiSwitchCDRMediatorShartel(i+1, Filename, sdrfile, isSecondary,
			 	                        	  //  		dir, destdir, sourceFileExt , destFileExt, seprator_value,
			 	                        	   ...
			 	                        	   // 		conn, 0, process );
			              	        			mediator = new TelesiSwitchCDRMediatorShartel(i+1, Filename, sdrfile, instanceid,
			 	                        	    		dir, destdir, sourceFileExt , destFileExt, seprator_value,
			 	                        	    		commit_after, parms,  debug, ne, ProcessUnSucc,
			 	                        	    		NodeHash, NodeIdentificationHash, BNumberRules, elementHash,
			 	                        	    		conn, 0, process,timeDiff,CDR_TIME_GMT);
