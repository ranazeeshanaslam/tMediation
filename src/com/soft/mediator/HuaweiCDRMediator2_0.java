 ...
 * @version 1.0
 */
public class HuaweiCDRMediator2_0 {
...
boolean isRunning = false;
    ...
    public static void main(String argv[]) throws IOException {
         ...
         parms.setErrSQLFilePath(argv[0]+"alarms/");
          ...
          ServerName = conf.getPropertyValue(conf.SERVER_NAME);
			...
			try{
				...
				HuaweiCDRMediator2_0 mediator = new HuaweiCDRMediator2_0();
				...
				//if (Util.validateSystem(conn, logger)){
					Records = mediator.mediateHuaweiCDRFiles2_0(conf, dbConnector, logger, parms);
				//}else{
      ...
      public long mediateHuaweiCDRFiles2_0(MediatorConf conf, DBConnector dbConnector, Logger logger, MediatorParameters parms) {
          ...
          try {
              ...
              if (!dir.isDirectory() || !destdir.isDirectory()) {
                  ...
                  for (int j = 0; j < FileNames.length; j++) {
                      ...
                      sourceFileExt=".dat";
                      logger.info("sourceFileExt = " + sourceFileExt);
		                              ...
		                              try {
		                            	  ...
		                            	  while(true)
		                                   { //#1
		                                      ...
		                                      HW_CSN = hex2dec(reverseBytes(HW_CSN.substring(0, 8), 2));
		                                      HW_BILLTYPE=hex2dec(extractField(1,7,buffer));
		                                      

		                                    String datestr=extractField(6,11,buffer);
		                           			String sdateTime = "20" + hex2dec(reverseBytes(datestr.substring(0, 2), 2)) + "-";
		                           			logger.debug("sdateTime :"+sdateTime);
		                           			...
		                           			SimpleDateFormat ft = new SimpleDateFormat ("yyyy-MM-dd HH:mm:ss");		//yyyy-MM-dd HH:mm:ss
		                        			Date t;
		                        			t = ft.parse(sdateTime);
		                        			
		                           			HW_STARTTIME="to_date('"+ sdateTime +"','YYYY-MM-DD HH24:MI:SS')";
		                           			logger.debug("HW_STARTTIME :"+HW_STARTTIME);
		                           			
		                           			String enddatestr=extractField(6,17,buffer);
		                           			String enddateTime = "20" + hex2dec(reverseBytes(enddatestr.substring(0, 2), 2)) + "-";
		                        			...
		                        			logger.debug("enddateTime :"+enddateTime);
		                        			Date t1;
		                        			t1 = ft.parse(enddateTime);
		                        			long tdate=t1.getTime()-t.getTime();
		                        			
		                        			//System.out.println("Start Date(Dec): " + sdateTime + " ---- End Date(Dec): " + enddateTime);
		                        			//System.out.println("Start Date: " + t + " ---- End Date: " + t1);
		                        			//System.out.println("\n\n\nStart Time (DateTime): " + DateTime.parse(sdateTime, DateTimeFormat.forPattern("yyyy-MM-dd HH:mm:SS")) + "\n\n");
		                        			//System.out.println("\n\n\nEnd Time (DateTime): " + new SimpleDateFormat ("yyyy-MM-dd HH:mm:SS").parse(enddateTime) + "\n\n");
		                        			
		                        			HW_ENDTIME="to_date('"+enddateTime +"','YYYY-MM-DD HH24:MI:SS')";
		                        			logger.debug("HW_ENDTIME :"+HW_ENDTIME);
...
//		                           			HW_CALLDURATION = Long.parseLong(Durstr);
		                           			...
		                           			String Durstr_Hex = extractField(4,23,buffer);
		                           			String Durstr_Dec = hex2dec(reverseBytes(Durstr_Hex.substring(0, 8), 2));
		                           			//Durstr=String.valueOf(parseDuration(Durstr));
		                           			//HW_CALLDURATION = Long.parseLong(Durstr);
		                        			HW_CALLDURATION=tdate/1000;
		                        			//System.out.println("\n\n\nDuration (Long): " + HW_CALLDURATION + "\n\n");
		                           			//System.out.println("\n\n\nDifference (Start-End): " + tdate + "\n\n");
		                           			
		                           			
		                           			Long raw_duration = Long.parseLong(Durstr_Dec);
		                           			double conversationTime_Double = Math.ceil((double)raw_duration / 100);
		                           			//double Duration_IF_ELSE = 0.0;
		                           			
//		                           			if(HW_CALLDURATION < 654) {
//		                           				Duration_IF_ELSE = Math.ceil(conversationTime_Double);
//		                           			} else {
//		                           				Duration_IF_ELSE = HW_CALLDURATION;
//		                           				if(Math.ceil(Duration_IF_ELSE) > Duration_IF_ELSE) {
//		                           					Duration_IF_ELSE += 1;
//		                           				}
//		                           				Duration_IF_ELSE = ((HW_CALLDURATION / 655) * 655) + conversationTime_Double;
//		                           				if((HW_CALLDURATION - Duration_IF_ELSE) == 1) {
//		                           					Duration_IF_ELSE += 1;
//		                           				} else if((HW_CALLDURATION - Duration_IF_ELSE) == 2) {
//		                           					Duration_IF_ELSE += 2;
//		                           				} else if((HW_CALLDURATION - Duration_IF_ELSE) == 3) {
//		                           					Duration_IF_ELSE += 3;
//		                           				}
//		                           			}
		                                    ...
		                                    if(!tgStr.contains("FFFF"))
		                                    	HW_TRUNKGROUPIN=Integer.parseInt(hex2dec(reverseBytes(tgStr.substring(0, 4), 2)));
		                                    String tgStrout=extractField(2,79,buffer);
		                                    if(!tgStrout.contains("FFFF"))
		                                    	HW_TRUNKGROUPOUT=Integer.parseInt(hex2dec(reverseBytes(tgStrout.substring(0, 4), 2)));
		                                    ...
		                                    HW_INROUTEID=AsString(getbytes(16,397,buffer));
		                                    HW_OUTROUTEID=AsString(getbytes(16,413,buffer));// extractField(16,413,buffer);
		                                    ...
		                                    //if (HW_CALLEDNUMBER != "" && HW_CALLEDNUMBER.length() != 0) {
                                        	    ...
                                        	    if (duplicate){
                                        	    	...
                                        	    	{
	                                        	    	ICPNode inode = Util.identifyICPNode(""+HW_TRUNKGROUPIN, "", "", HW_CALLEDNUMBER, HW_CALLEDNUMBER, true, ne, NodeIdentificationHash, NodeHash); 
		                                  	    		iNodeID = inode.getNodeID();
		                                  	    		ICPNode enode = Util.identifyICPNode(""+HW_TRUNKGROUPOUT, "", "", HW_CALLEDNUMBER, HW_CALLEDNUMBER, false, ne, NodeIdentificationHash, NodeHash); 
		                                  	    		eNodeID = enode.getNodeID();
		                                  	    		//logger.debug("CalledNumber="+CalledNumber);
		                                  	    		BNumberRuleResult result = Util.applyBNumberRules(orgCalledNumber, BNumberRules, inode, false, true);
...
//                                        	    	sql = " SELECT PAR_ISINTERNATIONAL FROM PAR_TBLINTERCONNECTPARTNERS " +
//                                        	    			" WHERE PAR_PARTNERID IN (SELECT PAR_PARTNERID FROM " +
//                                        	    			" PAR_TBLICPARTNERNODES WHERE PN_PARTNERNODEID = "+ iNodeID +" ) ";
//                                        	    	String Message = "No Ingress Partner Record Found";
//                                        	    	int ingress_IsInternational = 0, egress_IsInternational = 0;
//                                        	    	try {
...
//                                                		{
//                                                			while (flag) {
//                                                				ingress_IsInternational = rs.getInt("PAR_ISINTERNATIONAL");
//                                                				flag = rs.next();
//                                    						}
//                                                		} else {
...
//                                        	    	sql = " SELECT PAR_ISINTERNATIONAL FROM PAR_TBLINTERCONNECTPARTNERS " +
//                                        	    			" WHERE PAR_PARTNERID IN (SELECT PAR_PARTNERID FROM " +
//                                        	    			" PAR_TBLICPARTNERNODES WHERE PN_PARTNERNODEID = "+ eNodeID +" ) ";
//                                        	    	Message = "No Egress Partner Record Found";
//                                        	    	try {
//                                                        rs = stmt.executeQuery(sql);
//                                                        flag = rs.next();
//                                                		if(flag)
//                                                		{
//                                                			while (flag) {
//                                                				egress_IsInternational = rs.getInt("PAR_ISINTERNATIONAL");
//                                                				flag = rs.next();
//                                    						}
//                                                		} else {
//                                                			logger.debug(Message);
...
//                                                    }
//                                        	    	
//                                        	    	if(ingress_IsInternational == 1 && egress_IsInternational == 0) {	// IGW - ANS
//                                        	    		Duration_IF_ELSE += 1;
//                                        	    	}
                                        	    	
                                        	    	sql = " insert into SDR_TBLHUAWEICDRS ( HW_CSN, HW_NETTYPE, HW_BILLTYPE, HW_STARTTIME, HW_ENDTIME, HW_CALLDURATION, HW_CALLINGNUMBER, HW_CALLEDNUMBER, HW_TRUNKGROUPIN, HW_TRUNKGROUPOUT, HW_CALLTYPE, HW_TERMINCODE,ne_elementid, HW_CALLINGK, HW_CALLINGW, HW_CALLEDGK, HW_CALLEDGW,FN_FILEID,TSSW_INCOMINGNODEID,TSSW_OUTGOINGNODEID,FN_ISSECONDARY, HW_INROUTEID, HW_OUTROUTEID,HW_ORGCALLEDNUMBER " +
                                        	    			", RAW_DURATION, DIFF_DURATION ) " +
                                			    		" values ( '"+HW_CSN+"',  "+HW_NETTYPE+","+HW_BILLTYPE+","+HW_STARTTIME+","+HW_ENDTIME+", "+conversationTime_Double+", " +	// HW_CALLDURATION
                                			    		" '"+HW_CALLINGNUMBER+"', '"+TCalledNumber+"', "+HW_TRUNKGROUPIN+", "+HW_TRUNKGROUPOUT+"," +
                                			    		"  "+HW_CALLTYPE+", "+HW_TERMINCODE+", "+network_element+",'"+HW_CALLINGK+"','"+HW_CALLINGW+"','"+HW_CALLEDGK+"','"+HW_CALLEDGW+"',"+sdrfile.getFN_FILEID()+","+iNodeID+","+eNodeID+","+sdrfile.getFN_ISSECONDARY()+",'" + HW_INROUTEID +"', '"+ HW_OUTROUTEID +"','"+orgCalledNumber +"', " +
                                			    		" "+ raw_duration +", "+ HW_CALLDURATION + " ) ";
                                        	    	logger.debug(sql);
		//                            	    		int isExecuted = 0;
		                                	    	logger.debug(sql);
		                                	    	int isExecuted = 0;
...
//	public static String extractFieldBin(int iLength, int iOffset, java.nio.ByteBuffer ByteBuffer) {
//	    String result = "";
...
//        Logger log = Logger.getLogger("Mediation");
//        log.debug("\n\n\n\n" + intValue + "\n\n");
        if (intValue < 0) {
  ...
  public static int parseDuration(String duration) {
      ...
      return (int)Math.ceil(Integer.parseInt(duration) / 100);
  }
...
public static String reverseBytes(String strBytes, int blockSize) {
  ...
  public static byte[] getbytes(int iLength, int iOffset, java.nio.ByteBuffer ByteBuffer) {
	    byte[] bArr = new byte[iLength];
	    ByteBuffer.position(iOffset);
	    ByteBuffer.get(bArr, 0, bArr.length);
	   return bArr;
