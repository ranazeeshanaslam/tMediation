...
public class AsteriskmySQLCDRs {
		
	public AsteriskmySQLCDRs() {
    }
	...
	public  static void main( String argv[]) throws IOException , Exception
	{
        ...
        Hashtable elementHash;
      	...
      	try {
            ...
            String confFile = path +"conf/conf_asterisk.properties";
...
//        if (NoOfSources < 1) NoOfSources=1;
        	...
        	try{
				...
				AsteriskmySQLCDRs cdrfetcher = new AsteriskmySQLCDRs();
				...
				Enumeration elementlist =  elementHash.elements();
			 	while (elementlist.hasMoreElements()){
			 		...
			 		if (FromDate.length() > 0 && ToDate.length() > 0){
						whereclauseA += " and calldate > '"+FromDate+"' and calldate <= '"+ToDate+"'";
					} else {
						CDRFromToDate fromtoDate = cdrfetcher.getCDRFromToDate(conn, logger, ne.getElementID());
						if (fromtoDate.getFromDate().length() > 0){
							whereclauseA += " and calldate > '"+fromtoDate.getFromDate()+"'";
						}
						if (fromtoDate.getToDate().length() > 0){
							whereclauseA += " and calldate <= '"+fromtoDate.getToDate()+"'";
						}
					}
			 		...
			 		if (mc != null && mc.getIsSourceDB()==1){
			 			...
			 			if (mc.getDBType().length()>0 && mc.getDBDriver().length()>0 && mc.getDBURL().length()>0 && mc.getDBLogin().length()>0
			 				...
			 				try{
								...
								sql = 	" select calldate, clid, src, dst, dcontext, channel, dstchannel, lastapp," +
										" lastdata, duration, billsec, 0 as dispsition, amaflags, accountcode, uniqueid," +
										" userfield from "+ne.getNEMedConf().getDBTable()+" " +
										" where  "+whereclauseA+" ";
								...
								while(rs.next()){
										...
										if (dst == null ) dst="";
										cdr.setAccessNumber(dst);
										if(ne.getElementID() == 38)
											cdr.setIngressTrunk(dst);
										
										String dcontext = rs.getString("dcontext");
										if (dcontext == null) dcontext="";
										cdr.setContext(dcontext);
										
										String channel= rs.getString("channel");
										if (channel == null ) channel="";
										cdr.setSrcChannel(channel);
										
										String dstchannel= rs.getString("dstchannel");
										if (dstchannel == null ) dstchannel="";
										cdr.setDstChannel(dstchannel);
										
										String lastapp= rs.getString("lastapp");
										if (lastapp == null ) lastapp="";
										cdr.setLastApp(lastapp);
										...
										if(lastdata.length() <= 0)
											...
											//ZAP/G3/0172051893|30|HL(149907582000:61000:30000)
											logger.debug("lastdata ="+lastdata);
											if(lastdata.length() > 3){
												if (lastdata.substring(0,3).equalsIgnoreCase("ZAP")){
													try{
														CalledNumber = lastdata.substring(lastdata.indexOf("/", 4)+1, lastdata.indexOf("|", 0));
													}catch(Exception e){
														cdr = null;
														continue;
													}
										...
										long billsec = rs.getLong("billsec");
										if (rs.wasNull()) billsec=0;
										cdr.setBillSec(billsec);
										
										String dispsition= rs.getString("dispsition");
										...
										//lastapp, lastdata, duration, billsec, dispsition, amaflags, accountcode, uniqueid, userfield
										
										String uniqueid= rs.getString("uniqueid");
										if (uniqueid == null ) uniqueid="";
										cdr.setCallID(uniqueid);
										
										String userfield= rs.getString("userfield");
										if (userfield == null ) userfield="";
										cdr.setSessionID(userfield);
										
										if(ne.getElementID() == 37 && dcontext.equalsIgnoreCase("a2billingrs")){
											cdr = null;
											continue;
										}
